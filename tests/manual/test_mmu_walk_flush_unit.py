...
result must be thrown away instead of filled into the TLB: the access walks again afterwards and
sees what software just wrote.
...
gate's single pytest process crashes the simulator (`error -11`). From `tests/`:
    ...
    docker run --rm --platform linux/arm64 -v "$PWD/..:/repo" -v synapse32-gate-arm64:/work \
        -w /repo/tests synapse32-dev:arm64 bash -lc \
        'export PATH=/work/cache/venv/bin:$PATH; python -m pytest -q manual/test_mmu_walk_flush_unit.py'
"""
...
async def test_fence_during_a_walk_discards_its_result(dut):
    ...
    # A walk that a fence interrupts must not fill the TLB: the entry it read may be the one
    # software just replaced.
