    ...
    for i, instr in enumerate(instructions):
        ...
        try:
            ...
            if i < 3 or i >= len(instructions) - 2:
                print(f"[DEBUG]   [{i:3d}] 0x{byte_addr:08x}: 0x{instr:08x}")
            elif i == 3:
...
async def test_memory_intensive_workload(dut):
    ...
    instructions.extend([
        ...
        # Wait for store queue to drain
        NOP(),               # 11: Wait for stores to complete
        NOP(),               # 12:
        NOP(),               # 13:
        NOP(),               # 14:

        # Read loop starts at index 15
        LW(3, 10, 0),        # 15: x3 = MEM[x10]
        NOP(),               # 16: Wait for load queue
        NOP(),               # 17:
        NOP(),               # 18:
        NOP(),               # 19:
        ADD(5, 5, 3),        # 20: sum += x3
        ADDI(10, 10, 4),     # 21: x10 += 4
        ADDI(1, 1, 1),       # 22: index++
        NOP(),               # 23: Placeholder for branch
    ])

    # Calculate read loop branch: from index 23 to index 15
    read_loop_offset = (15 - (23 + 1)) * 4  # (target - (branch_idx + 1)) * 4
    instructions[23] = BNE(1, 2, read_loop_offset)

    instructions.append(HALT())  # 24: Done
    ...
    for cycle in range(0, max_cycles, 100):
        ...
        pc = int(dut.pc_debug.value)
        if pc == 0x60:  # Halt address (instruction 24 * 4 = 0x60)
            print(f"[INFO] Program completed at cycle {cycle}")
            break
