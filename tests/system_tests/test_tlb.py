...
# phase -> PTE reads that phase must take. A flush (SFENCE.VMA or a write to satp) drops every
# translation, so the next instruction fetch walks the code megapage again and the next mark store
# walks the data megapage again; those reads are counted in the phase they happen in.
EXPECTED_WALK_READS = {
...
async def test_walker_and_tlb(dut):
    ...
    for _ in range(20000):
        ...
        await ReadOnly()
        # A PTE read is a read of the page-table region on the backing store's data port. The
        # combinational walker used private ports instead, so it never appeared here.
