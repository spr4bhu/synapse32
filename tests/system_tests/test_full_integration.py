...
async def load_program(dut, instructions, start_addr=0):
    ...
    for i, instr in enumerate(instructions):
        ...
        try:
            ...
            print(f"[ERROR] Failed to load instruction at word_addr {word_addr}: {e}")
            break
...
def get_data_mem(dut, addr):
    """Get data from data memory (byte-addressable)"""
    try:
        # data_mem uses byte addressing directly (relative to DATA_MEM_BASE)
        rel_addr = addr - DATA_MEM_BASE
        data_ram = dut.data_mem_inst.data_ram
        # Read 4 bytes and combine (little-endian)
        b0 = int(data_ram[rel_addr].value)
        b1 = int(data_ram[rel_addr + 1].value)
        b2 = int(data_ram[rel_addr + 2].value)
        b3 = int(data_ram[rel_addr + 3].value)
        val = (b3 << 24) | (b2 << 16) | (b1 << 8) | b0
        print(f"[DEBUG] Read MEM[0x{addr:08x}] = 0x{val:08x}")
        return val
    except Exception as e:
...
def set_data_mem(dut, addr, value):
    """Set data in data memory (word, byte-addressable storage)"""
    try:
        rel_addr = addr - DATA_MEM_BASE
        data_ram = dut.data_mem_inst.data_ram
        # Write 4 bytes (little-endian)
        data_ram[rel_addr].value = value & 0xFF
        data_ram[rel_addr + 1].value = (value >> 8) & 0xFF
        data_ram[rel_addr + 2].value = (value >> 16) & 0xFF
        data_ram[rel_addr + 3].value = (value >> 24) & 0xFF
        print(f"[DEBUG] Set MEM[0x{addr:08x}] = 0x{value:08x}")
    except Exception as e:
